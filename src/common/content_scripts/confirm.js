chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
	console.log("GOT A MESAGE", request)
	switch (request.command) {
		case "showSetTriveBountyModal":
			alertify.confirm("Would you like to set a bounty of 1 Trive token to help people get started?", function () {
				...
				// sendResponse({ message: "confirmedSetTriveBounty" })
			}, function () {
				sendResponse({})
			})
			break;
		case "showSetTriveBountyModalSuccess":
			console.log(request.data)	
			if (request.data && request.data.already_created)
				alertify.alert("Bounty has already been set for this page")
			else
				alertify.alert("Bounty created successfully");
			break;
		case "showSetTriveBountyModalFailure":
			console.log("afial is ", request);	
			if (request.error && request.error.json && request.error.json.error == "not-logged-in") {
				alertify.alert("You must be logged in to submit an article");
				return;
			}	
			alertify.alert("There was an error creating the Trive bounty for this article")
			break;	
	}
});
...
function getDocumentHref(){
	...
	var allowedSchemes = ['http:', 'https:', 'file:']
	
	if (allowedSchemes.indexOf(window.location.protocol) > -1) {
		return href;
	} else {
	...
	function Alertify() {
		...
		var _alertify = {
			...
			setup: function (item) {
				...
				el.className = "trive-plugin-alertify hide";
